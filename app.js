// WAJIB menggunakan ekstensi .js di belakang impor agar browser/Vercel tidak error 404
import { jalankanLoader } from './loader.js';
import { ALL_DATA, SOLAR_DATA, DEEP_SPACE_DATA, MAIN_VIEWS, EXTRA_VIEWS } from './data.js';
import * as Draw from './drawings.js';

// Menunggu kerangka HTML dimuat
document.addEventListener("DOMContentLoaded", () => {
    // 1. Eksekusi Loader secara mandiri
    jalankanLoader(() => {
        // 2. Saat loader selesai, hidupkan alam semesta
        initSimulasi();
    });
});
...
function initSimulasi() {
    ...
    // Pastikan Canvas Tidak 0 pixel (Blank)
    function resize() { 
        canvas.width = window.innerWidth; 
        canvas.height = window.innerHeight; 
    }
    window.addEventListener('resize', resize);
    resize(); // Eksekusi langsung ukuran pertama
    ...
    // --- Looping Animasi Kanvas Utama ---
    function loop() {
      const w = canvas.width, h = canvas.height;
      const cx = w / 2, cy = h / 2;
      ctx.clearRect(0, 0, w, h);

      // Rendering Objek Berdasarkan View Saat Ini
